...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("chefType").addEventListener("change", function () {
      ...
      const cuisines = chefType === "domestic"
  ...
  window.showChefModal = function () {
      ...
      document.getElementById("membersCount").textContent = numMembers;
      document.getElementById("membersPrice").textContent = memberPrice;
      ...
      fetch(`${API_BASE_URL}/chefbookings/create`, {
          ...
          .then((data) => {
              if (data.status === "SUCCESS" && data.sessionUrl) {
                  ...
                  throw new Error(data.message || "Unexpected error occurred while creating the payment session.");
              }
          })
          .catch((error) => {
...
function proceedToPay() {
  ...
  .then((response) => {
      if (!response.ok) {
          throw new Error("Failed to process the payment request. Please try again.");
      }
      return response.json();
  })
  .then((data) => {
